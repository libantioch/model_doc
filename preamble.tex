...
\gdef\unit@parser{\cdot}
\newcommand{\print@unitOrSpace}[2]{%
\ifnum#1=0#2\else{#2}\fi
}
\newcommand{\parse@unit}[2][0]{%
\def\go@next[#1]{}%
\ifx#2\relax%%
\else%
  \let\go@next=\parse@unit%
  \print@unitOrSpace{#1}{#2}%
\fi%
\newcount\ans
\if^#1\ans=1\else\ans=0\fi
\go@next[\the\ans]%
}
\newcommand{\unit}[1]{\ensuremath{\mathtt{\parse@unit#1\relax}}}
