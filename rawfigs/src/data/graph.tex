...
\node[below left = 2cm and 10cm] (DiffSet)    at (WilMix)  {\obj[DiffusionModel,Mixture]{PhysicalSet}};
...
\node[left =  4cm] (ThermoEval) at (core.west) {\obj[MacroThermo,MicroThermo]{ThermoEvaluator}};
% Macro
...
%%% Stockmayer
\begin{scope}[every node/.append style={transportGen},every path/.append style={transportGen}]
\node (Stock)  at (LewisDiff |- {KinTVis -| BinMolDiff})  {\obj{StockmayerPotential}};
%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
%%
%%% diff
...
\draw[ref]   (DiffSet.5) |- (WilMix);
\draw[uses]  (Stock)     -| (BinMolDiff);
\end{scope}
%% viscosity
\begin{scope}[every node/.append style={viscosityGen},every path/.append style={viscosityGen}]
\draw[templ]   ([xshift=-5pt]TranMix.-10) |- ([xshift=-5pt,yshift=-5pt]$(ChemSpec.south west)!0.5!(TranSpec.north east)$)  -| ([xshift=15pt]VisSet.north);
\draw[ref]   (VisSet.north -| WilMix.south) -- (WilMix.south);
\draw[uses]  (Stock)  -- (KinTVis);
\end{scope}
%% thermal conduction
\begin{scope}[every node/.append style={thermalCondGen},every path/.append style={thermalCondGen}]
\draw[templ]   ([xshift=5pt]TranMix.-10) |- ([xshift=5pt,yshift=5pt]$(ChemSpec.south west)!0.5!(TranSpec.north east)$) -| ([xshift=5pt]TauSet.north);
\draw[ref]   (TauSet) |- (WilMix);
\end{scope}
\end{scope}
%%%% thermo
\begin{scope}[every node/.append style={thermoGen},every path/.append style={thermoGen}]
